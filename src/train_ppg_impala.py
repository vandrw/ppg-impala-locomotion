...
def main(args):
    ...
    logging.basicConfig(filename=os.path.join(output_path, "train.log"), stream=sys.stdout, filemode='w', level=logging.INFO)
    ...
    if continue_run:
        logging.info("Found previous model in {}. Continuing training.".format(output_path))
    ...
    if args.log_wandb:
        try:
            ...
            logging.error("You've requested to log metrics to wandb but package was not found. "
    ...
    try:
        runners = [
            Runner.remote(args.env, args.train_mode, args.visualize, args.n_update, i, save_path=output_path)
